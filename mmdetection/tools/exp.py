...
class Boxes(object):
    ...
    def draw_rectangle(self, bboxes, img,msg=None,color=(0,255,0), dim=(200,200)):
        try:
            for bbox in bboxes:
                if len(bbox) > 1:
                    bbox = bbox[0]
                ...
                if msg is not None:
                    img = cv2.putText(img,msg,point_1, cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, color , 1, cv2.LINE_AA)
            return img
        except ValueError as error:
            print(error)
    ...
    def fromJson(self,path, images, labels=('pig',)):
        try:
            ...
            a = ground.copy()
            b = ground.copy()
            self.metrics(images ,a ,b)
            return ground
        except ValueError as error:
            print(error)

    def metrics(self, path, annots, preds, threshold=0.8):
        try:
            ...
            for key, values in imgs.items():
                ...
                #img = self.draw_rectangle(values['tp'] ,img,msg='tp:pig',dim=dim,color=(0,255,0))
                #img = self.draw_rectangle(values['fp'] ,img,msg='tp:pig',dim=dim,color=(255,255,0))
                #img = self.draw_rectangle(values['fn'] ,img,msg='tp:pig',dim=dim,color=(0,0,255))
                img = self.draw_rectangle( annots[key]['bboxes'], img, msg='tp:pig', dim=dim,color=(255,0,0))
    ...
    def more_close(self, annots, pred, threshold=0.8):
        try:
            ...
            for key, value in annots.items():
                ...
                imgs[key] = {'tp':tp, 'fn':fp,'fp':box_pred,'dim':(value['width'],value['height'])}
            return imgs
        except ValueError as error:
            print(error)
