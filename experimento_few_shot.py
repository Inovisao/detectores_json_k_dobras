...
import mmfewshot
print('mmfewshot: ',mmfewshot.__version__)
import mmcv
...
from mmfewshot.datasets import build_dataset,build_dataloader
from mmfewshot.models import build_detector
from mmfewshot.apis import train_detector
from mmfewshot import __version__
from mmfewshot.utils import collect_env, get_root_logger
from mmcv.utils import get_git_hash
import torch
import os.path as osp
import numpy as np
from mmfewshot.apis import inference_detector, init_detector, show_result_pyplot
from mmfewshot.datasets import replace_ImageToTensor
from mmfewshot.datasets import CocoDataset
from mmcv.visualization import color_val
from mmfewshot.core import visualization as vis
import mmcv, torch
import cv2
...
sys.path.append('mmfewshot')

pasta_mmfewshot=os.path.join(os.getcwd(),'mmfewshot')
pasta_dataset=os.path.join(os.getcwd(),'dataset')
pasta_checkpoints=os.path.join(os.getcwd(),'checkpoints')
print('Pasta do mmfewshot: ',pasta_mmfewshot)
...
#
# É preciso também baixar o arquivo .pth no site do mmfewshot e colocar dentro da
# pasta mmfewshot/checkpoints. Os arquivos .pth para rede vfnet, por exemplo, podem ser
# encontrados no link abaixo:
# https://github.com/open-mmlab/mmfewshot/blob/master/configs/vfnet/README.md
#
...
MODELS_CONFIG = {
    'tfa':{
        'config_file': 'configs/detection/tfa/coco/tfa_r101_fpn_coco_base-training.py',
        'checkpoint': pasta_checkpoints+'/tfa_r101_fpn_voc-split3_base-training_20211031_114840-fd8a9864.pth'
    },
}
...
def setCFG(selected_model,
  ...
  config_file = os.path.join(pasta_mmfewshot,MODELS_CONFIG[selected_model]['config_file'])
  ...
  from mmfewshot.apis import set_random_seed
  ...
  cfg.data.test.ann_cfg = [dict(type='ann_file',ann_file='filesJSON/'+fold+'_test.json')],
  ...
  cfg.data.train.ann_cfg = [dict(type='ann_file',ann_file='filesJSON/'+fold+'_train.json')],
  cfg.data.train.num_novel_shots = 1,
  cfg.data.train.num_base_shots = 5,
  ...
  cfg.data.val.ann_cfg = [dict(type='ann_file',ann_file='filesJSON/'+fold+'_val.json')],
